...
func withProfileLock(profile string, fn func() error) error {
	...
	lockPath := filepath.Join(zentorisDir(), "credentials-"+normProfile(profile)+".lock")
	...
	if err != nil {
		return fmt.Errorf("acquire profile lock %s: %w", lockPath, err)
	}
	if !locked {
		return fmt.Errorf("acquire profile lock %s: timed out", lockPath)
	}
