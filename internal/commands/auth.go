...
func newAuthLogoutCmd(d *deps) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(c *cobra.Command, _ []string) error {
			...
			if err := auth.UnregisterLogout(d.cfg.Profile); err != nil {
				return err
			}
...
func newAuthListCmd(d *deps) *cobra.Command {
	return &cobra.Command{
		Use:   "list",
		Short: "List the profiles you have logged in",
		Args:  cobra.NoArgs,
		RunE: func(c *cobra.Command, _ []string) error {
			infos, err := auth.ListProfiles()
			if err != nil {
				return err
			}
			if len(infos) == 0 {
				fmt.Fprintln(c.OutOrStdout(), "No profiles yet. Run `zentoris auth login`.")
				return nil
			}
			// A leading "*" marks the active default; other profiles need --profile or `auth switch`.
			// Each row is `<profile> (<store>) - <account identity>`.
...
func newAuthSwitchCmd(d *deps) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			fmt.Fprintf(c.OutOrStdout(), "Active profile is now %q.\n", args[0])
			return nil
		},
	}
}
...
func newAuthStatusCmd(d *deps) *cobra.Command {
	return &cobra.Command{
		...
		Args:  cobra.NoArgs,
		RunE: func(c *cobra.Command, _ []string) error {
			fmt.Fprintf(c.OutOrStdout(), "Active profile: %s\n", d.cfg.Profile)
			...
			// one, which account identity it holds, so the user knows who they are acting as.
			if src == "login" {
				store := auth.NewStore()
				fmt.Fprintf(c.OutOrStdout(), "Credentials stored in: %s.\n", store.Backend(d.cfg.Profile))
				if creds, _ := store.Load(d.cfg.Profile); creds != nil && creds.Subject != "" {
